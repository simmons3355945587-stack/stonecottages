...
(() => {
    ...
    <section class="stone-preference-section">
      ...
      <div style="margin-bottom:4px;">
        ...
        <select id="stoneLangModePref" style="width:100%; padding:8px 10px; font-size:13px; border-radius:var(--radius-sm); border:1.5px solid var(--paper-border); background:var(--paper-surface-sub); color:var(--text-primary); cursor:pointer;">
          <option value="zh">🇨🇳 中文详细释义优先（抽屉词典优先展示中文，词库纯英背词）</option>
          <option value="en">🇬🇧 纯英权威词典优先（抽屉词典优先展示英文释义）</option>
        </select>
      </div>
    </section>
    `:''}
  ...
  document.getElementById('stoneLangModePref')?.addEventListener('change', (e) => {
    if (typeof appSettings !== 'undefined' && appSettings) {
      ...
      if (typeof showToast === 'function') {
        showToast(appSettings.showChinese ? '🇨🇳 词典抽屉已设为【中文详细释义优先】' : '🇬🇧 词典抽屉已设为【纯英权威词典优先】');
      }
    }
  });
  ...
  if(!listening){
    ...
    const updateHeroCount=()=>{const c=document.getElementById('filterResultCountChip'),h=document.getElementById('heroStudyCount');if(c&&h)h.textContent=c.textContent.replace(/[^\d]/g,'')||'3837';};
    ...
    let isRefreshingMark=false;
    const refreshMark=()=>{
      if(isRefreshingMark)return;
      if(typeof currentLookupWord==='undefined'||!currentLookupWord)return;
      isRefreshingMark=true;
      try{
        const count=typeof getMarkCount==='function'?getMarkCount(currentLookupWord):0;
        const addBtn=document.getElementById('stoneDrawerMarkAdd')||document.getElementById('stoneDrawerMark');
        if(addBtn)addBtn.textContent='★ +1 · '+count;
        const minusBtn=document.getElementById('stoneDrawerMarkMinus');
        if(minusBtn){
          minusBtn.disabled=(count<=0);
          minusBtn.style.opacity=(count<=0)?'0.4':'1';
          minusBtn.title=(count<=0)?'未标记':'[-] 减少标记 (Mark -1)';
        }
