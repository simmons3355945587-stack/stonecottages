  ...
  const ExpeditionEngine = {
    ...
    async enterNode(nodeId, candidateQuestions = []) {
      ...
      if (Array.isArray(currentSession.reviewQueue) && currentSession.reviewQueue.length > 0 && node.type !== 'gatekeeper') {
        ...
        if (rIdx !== -1) {
          ...
          // 动态验证与清洗历史残余异常题目 (杜绝历史旧版本本地缓存的残缺/选项自猜题目)
          const validator = (typeof StoneQuestionValidator !== 'undefined') ? StoneQuestionValidator : (typeof window !== 'undefined' ? window.StoneQuestionValidator : null);
          if (validator && reviewItem && reviewItem.word) {
            const vRes = validator.validateQuestion(reviewItem, reviewItem.word);
            if (!vRes.valid) {
              const fresh = validator.buildCalibratedQuestion({
                targetWord: reviewItem.word,
                correctZh: reviewItem.translation || '核心考点释义',
                currentSan: currentSession.san || 100
              });
              if (fresh) {
                reviewItem.options = fresh.options;
                reviewItem.correctIndex = fresh.correctIndex;
                reviewItem.challengeType = fresh.challengeType;
                reviewItem.challengeTier = fresh.challengeTier;
                reviewItem.promptType = fresh.promptType;
                reviewItem.clozeSentence = fresh.clozeSentence;
                reviewItem.clozeTranslation = fresh.clozeTranslation;
                reviewItem.metaNotes = fresh.metaNotes;
              }
            }
          }
    ...
    async submitAnswer(selectedIndex, usedHint = false) {
      ...
      if (isCorrect) {
        ...
        // 考点熟练掌握：生词本标记减 1 (受系统设置 expeditionAutoReduceMark 控制，默认答对减 1，可设置为不减 1)
        const shouldReduce = (typeof appSettings !== 'undefined' && appSettings)
          ? (appSettings.expeditionAutoReduceMark !== false)
          : (() => {
              try {
                const raw = (typeof StoneStorage !== 'undefined' ? StoneStorage.getItem('vocab_settings') : null) ||
                            (typeof localStorage !== 'undefined' ? localStorage.getItem('vocab_settings') : null);
                if (raw) {
                  const parsed = JSON.parse(raw);
                  return parsed.expeditionAutoReduceMark !== false;
                }
              } catch (e) {}
              return true;
            })();
        ...
        if (shouldReduce) {
          const reduceFn = (typeof reduceMark === 'function') ? reduceMark : (typeof window !== 'undefined' ? window.reduceMark : null);
          if (typeof reduceFn === 'function' && encounter.word) {
            reduceFn(encounter.word);
          }
        }
      } else {
        ...
        // 考点失误：自动记入生词本，标记加 1
        const addFn = (typeof addMark === 'function') ? addMark : (typeof window !== 'undefined' ? window.addMark : null);
        if (typeof addFn === 'function' && encounter.word) {
          addFn(encounter.word);
        }
