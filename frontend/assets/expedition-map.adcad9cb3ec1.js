...
(function(root, factory) {
  ...
  const ExpeditionMap = {
     ...
     */
    _buildChallengeQuestion(targetWord, correctZh, wordPool = [], currentSan = 100, isListening = false) {
      if (typeof StoneQuestionValidator !== 'undefined' && typeof StoneQuestionValidator.buildCalibratedQuestion === 'function') {
        const graph = (typeof STONE_LEXICAL_GRAPH !== 'undefined') ? STONE_LEXICAL_GRAPH : {};
        return StoneQuestionValidator.buildCalibratedQuestion({
          targetWord,
          correctZh,
          wordPool,
          currentSan,
          graph,
          isListening,
          collectDistractors: (tw, cz, wp) => this._collectDistractors(tw, cz, wp)
        });
      }
    ...
    async getCandidateQuestionsForNode(nodeId, node) {
      ...
      if (wordSource === 'novel_ihopethisfindsyouwell' && !markedOnly && typeof Stone !== 'undefined' && typeof Stone.chapter === 'function') {
        ...
        if (chapterData && Array.isArray(chapterData.beats) && chapterData.beats.length > 0) {
          ...
          for (const beat of selectedBeats) {
            ...
            const challenge = this._buildChallengeQuestion(targetWord, correctZh, beats.map(b => b.targetWord), currentSan, !!node.isListening);
            candidates.push({
              word: targetWord,
      ...
      for (const targetWord of selectedWords) {
        ...
        const challenge = this._buildChallengeQuestion(targetWord, correctZh, rawWords, currentSan, !!node.isListening);

        let storyEn = challenge.story;
        let storyCn = challenge.story_cn;

        if (node.isListening) {
          storyEn = `[TRANSMISSION INTERCEPT] Tactical beacon broadcasting on encrypted frequency. Listen closely to the audio transmission and identify the corresponding meaning.`;
          storyCn = `【截获荒原加密电波】戴上耳机仔细辨听原声单词发音，在下方选出正确的考点义项以破译电波加密！`;
        } else if (!storyEn || !storyCn) {
          if (challenge.clozeSentence) {
            storyEn = `[TACTICAL SITUATION · CONTEXT BRIEFING] Field telemetry recorded an operational log: "${challenge.clozeSentence}". Evaluate the situational nuances to determine the exact term.`;
            storyCn = `【战术现场 · 语境研判】前哨侦察小队带回了一段关键调查日志。结合语境因果与搭配逻辑，准确选出契合句意的关键考点词汇。`;
          } else {
            storyEn = `[TACTICAL ENCOUNTER · DEFENSIVE POST] The frontier team reached an ancient security console. Accurately verify the target term's authentic operational meaning to proceed safely.`;
            storyCn = `【废墟前哨 · 封锁排查】前沿探险队在狭窄走廊遭遇了古老的防御控制台。准确辨析其汉语释义以解除禁制继续出征。`;
          }
        }
    ...
    updateHud(session) {
      ...
      if (typeof playerProfile !== 'undefined' && playerProfile) {
        ...
        // 直接兜底更新顶部全局 HUD DOM，杜绝状态脱节 (如 92 vs 100)
        const globalHudHp = document.getElementById('hudHp');
        if (globalHudHp) {
          const grayTxt = grayHp > 0 ? ` (+${grayHp})` : '';
          globalHudHp.textContent = `${hp}${grayTxt}`;
        }
        const globalHpBar = document.getElementById('hpBar');
        if (globalHpBar) globalHpBar.style.width = `${Math.max(0, Math.min(100, hp))}%`;
        const globalHpGrayBar = document.getElementById('hpGrayBar');
        if (globalHpGrayBar) {
          const totalWithGray = Math.min(100, (hp || 0) + (grayHp || 0));
          globalHpGrayBar.style.width = `${totalWithGray}%`;
        }
        const globalHudSan = document.getElementById('hudSan');
        if (globalHudSan) {
          const sanTier = (typeof GrayHealthEngine !== 'undefined') 
            ? GrayHealthEngine.getSanPollutionTier(san) 
            : { label: '清明' };
          globalHudSan.innerHTML = `<span class="hud-num">${san}</span><span class="hud-san-tag"> · ${sanTier.label}</span>`;
        }
        const globalSanBar = document.getElementById('sanBar');
        if (globalSanBar) globalSanBar.style.width = `${Math.max(0, Math.min(100, san))}%`;
      }
    },
    ...
    openEncounterModal(encounter, node) {
      ...
      if (isBoss) {
        ...
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(239, 68, 68, 0.12); border:1.5px solid rgba(239, 68, 68, 0.45); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            <span style="font-size:24px;">🧠</span>
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#ef4444;">【CORRUPTED · 认知污染】 (SAN ${currentSan})</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">搭配与语境边界模糊！请根据句式前后因果与习惯用法锁定唯一正解。</div>
            </div>
          </div>
        `;
        ...
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(147, 51, 234, 0.12); border:1.5px solid rgba(147, 51, 234, 0.4); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            ...
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#a855f7;">【DISTORTED · 语义扭曲】 (SAN ${currentSan})</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">近义易混词族干扰增强，请结合句意深度辨析细微内涵。</div>
            </div>
          </div>
        `;
      } else if (isClozeForm) {
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(245, 158, 11, 0.12); border:1.5px solid rgba(245, 158, 11, 0.4); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            ...
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#f59e0b;">【HAZE · 轻度雾化】 (SAN ${currentSan})</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">词形微眩，选项转化为 4 个形近英文单词，请仔细辨析拼写与词根！</div>
            </div>
          </div>
        `;
      } else if (isPolluted) {
        bannerHtml = `
          <div class="expedition-mist-banner">
            ...
            <div>
              <strong>【迷雾异象压制】</strong> 溪谷低语弥漫，能见度降低，请凝神辨析核心义项！
            </div>
          </div>
        `;
      }
      ...
      let wordDisplayHtml = '';
      if (isListening) {
        // 听力模式：绝对禁止在答前渲染单词文本与音标，防止“不听也能答”
        wordDisplayHtml = `
          <div style="text-align:center; margin-bottom:16px; padding:14px 18px; background:var(--paper-surface); border:1.5px dashed var(--brand-primary); border-radius:var(--radius-md);">
            <div style="font-size:14px; font-weight:900; color:var(--brand-primary); margin-bottom:6px;">
              🎧 考点原声音频已就绪
            </div>
            <div style="font-size:12px; color:var(--text-secondary); line-height:1.5;">
              荒原迷雾遮蔽了考点文字，请点击上方“🔊 辨听原声发音”后，在下方选出正确含义。
            </div>
          </div>
        `;
      } else if (isCloze && encounter.clozeSentence) {
        ...
        if (res.defeated) {
          ...
          resArea.innerHTML = `
            <div style="background:${res.isCorrect ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.08)'}; border:1px solid ${res.isCorrect ? 'var(--brand-success)' : 'var(--brand-danger)'}; border-radius:var(--radius-md); padding:12px 14px; margin-bottom:14px;">
              ...
              ${(!res.isCorrect && selectedIdx >= 0) ? `
                <div style="margin-top:8px; padding:8px 10px; background:rgba(239, 68, 68, 0.06); border-radius:var(--radius-sm); font-size:12px; color:var(--text-primary); line-height:1.5; border-left:3px solid var(--brand-danger); text-align:left;">
                  <strong style="color:var(--brand-danger);">⚠️ 选项甄别诊断：</strong> 你选择了【<strong>${escapeHtml(encounter.options[selectedIdx] || '')}</strong>】。本句关键考查的是【<strong>${escapeHtml(encounter.word)}</strong>】在此特定语境中的专属含义与搭配逻辑。
                </div>
              ` : ''}
              ...
              ` : ''}
              ${encounter.story_cn ? `
                <div style="margin-top:8px; padding:8px 10px; background:var(--paper-surface); border-radius:var(--radius-sm); font-size:12px; color:var(--text-secondary); line-height:1.5; border-left:3px solid var(--brand-primary); text-align:left;">
                  <strong style="color:var(--brand-primary);">🌐 背景语境对照：</strong>${escapeHtml(encounter.story_cn)}
                </div>
              ` : ''}
              ${encounter.metaNotes ? `
