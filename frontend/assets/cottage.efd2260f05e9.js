...
function renderExpeditionPrepContent(modalEl) {
    ...
    if (c) {
      return `
        <div style="background:var(--paper-surface-sub); border:1.5px solid var(--brand-primary); border-radius:var(--radius-sm); padding:6px 4px; text-align:center;">
          ...
          <div style="font-size:16px; margin:1px 0;">${c.icon}</div>
          <div style="font-weight:800; font-size:11px; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${escapeHtml(c.arcana || c.word)}</div>
          <div style="font-size:9.5px; color:var(--brand-primary); font-weight:700;">${escapeHtml(c.skill)}</div>
        </div>
      `;
    } else {
      return `
  ...
  const totalScopeWords = rawScopeWords.length || 480;
  ...
  modalEl.innerHTML = `
    ...
    <div style="margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
        <span style="font-size:11.5px; font-weight:800; color:var(--text-primary);">🎴 携带策略手牌 (3/3)</span>
