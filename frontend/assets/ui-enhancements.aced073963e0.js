...
(() => {
  ...
  dialog.innerHTML=`<div class="stone-dialog-heading"><h2 id="stonePreferencesTitle">阅读与学习设置</h2><button type="button" data-close aria-label="关闭设置">×</button></div>
    ...
    ${!listening?'<button type="button" id="stoneAdvanced">高级设置</button>':''}`;
  document.body.append(dialog);
  const updateLabels=()=>{for(const key of ['words','story','listening']){const size=Math.max(16,Math.min(24,Number(preferences[key])||18));document.getElementById('stoneSize-'+key).textContent=size+' px';dialog.querySelector(`[data-size="${key}"][data-step="-1"]`).disabled=size<=16;dialog.querySelector(`[data-size="${key}"][data-step="1"]`).disabled=size>=24;}};
  updateLabels();
  ...
  const mobileNav=document.createElement('nav');mobileNav.className='stone-mobile-nav';mobileNav.setAttribute('aria-label','主要功能');mobileNav.innerHTML=`<a href="/" data-main-nav="words">${icon('words')}<span>词库</span></a><a href="/cet4/" data-main-nav="listening">${icon('listening')}<span>精听</span></a><button data-main-nav="games">${icon('games')}<span>闯关</span></button><button data-main-nav="mine">${icon('mine')}<span>我的</span></button>`;document.body.append(mobileNav);
  ...
  if(!listening){
    ...
    for(const [id,name,label] of [['navWords','words','词库'],['navSurvival','games','小说闯关'],['navMatch','games','消消乐'],['navProfile','games','塔罗卡牌'],['navCet4','listening','四级精听']]){const b=document.getElementById(id);b.querySelector('.nav-icon').innerHTML=icon(name);b.querySelector('span:last-child').textContent=label;}
    ...
    const gameNav=document.createElement('nav');gameNav.className='stone-game-subnav';gameNav.setAttribute('aria-label','闯关方式');gameNav.innerHTML='<button data-game="survival">小说闯关</button><button data-game="match">消消乐</button><button data-game="profile">塔罗卡牌</button>';document.querySelector('.container').prepend(gameNav);gameNav.onclick=e=>{const b=e.target.closest('[data-game]');if(b)switchNavView(b.dataset.game);};
    const syncNavigation=()=>{const active=document.querySelector('.view-section.active')?.id;const name={viewWords:'words',viewSurvival:'survival',viewMatch:'match',viewProfile:'profile'}[active]||'words';document.body.dataset.activeView=name;if(name!=='words'){preferences.lastGame=name;save();}mobileNav.querySelectorAll('[data-main-nav]').forEach(b=>{if(b.dataset.mainNav===(name==='words'?'words':'games'))b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});gameNav.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.game===name)));};
    ...
    const footer=document.createElement('div');footer.className='stone-dictionary-actions';footer.innerHTML='<button data-adjacent="-1" aria-label="上一个单词">← 上一个</button><button id="stoneDrawerMark">★ 标记 +1</button><button data-adjacent="1" aria-label="下一个单词">下一个 →</button>';
    drawer.append(footer);
    footer.onclick=e=>{const b=e.target.closest('button');if(!b||typeof currentLookupWord==='undefined'||!currentLookupWord)return;if(b.dataset.adjacent){const w=getAdjacentWord(currentLookupWord,Number(b.dataset.adjacent));if(w)openWordDetails(w);}else{addMark(currentLookupWord);refreshMark();}};
    const refreshMark=()=>{if(typeof currentLookupWord==='undefined'||!currentLookupWord)return;document.getElementById('stoneDrawerMark').textContent='★ 标记 +1 · '+getMarkCount(currentLookupWord);};
