...
(function(root, factory) {
  ...
  const ExpeditionMap = {
     ...
     * SAN < 20: 狂乱绝境 (english_illusion) - 中文指引瓦解，纯英单词幻象对决
     */
    _buildChallengeQuestion(targetWord, correctZh, wordPool = [], currentSan = 100) {
      ...
      // 1. SAN < 20: 狂乱绝境 · 纯英全真幻象 (English Illusion)
      if (currentSan < 20) {
        let engOptions = [targetWord];
        let notesList = [];

        if (entry) {
          if (Array.isArray(entry.spelling_confusables)) {
            for (let item of entry.spelling_confusables) {
              if (engOptions.length < 4 && !engOptions.includes(item.word)) {
                engOptions.push(item.word);
                notesList.push(`${item.word} (${item.zh || '形近'})`);
              }
            }
          }
          if (Array.isArray(entry.synonyms)) {
            for (let item of entry.synonyms) {
              if (engOptions.length < 4 && !engOptions.includes(item.word)) {
                engOptions.push(item.word);
                notesList.push(`${item.word} (${item.zh || '近义'})`);
              }
            }
          }
        }
        ...
        // 备用：从候选词池补足 4 个英文词
        const candidates = (Array.isArray(wordPool) && wordPool.length > 0) ? wordPool : Object.keys(cDict);
        for (let i = 0; i < candidates.length && engOptions.length < 4; i++) {
          const cand = candidates[(i * 7 + 1) % candidates.length];
          if (cand && !engOptions.includes(cand)) {
            engOptions.push(cand);
          }
        }
        while (engOptions.length < 4) {
          engOptions.push('option_' + engOptions.length);
        }

        return {
          options: engOptions,
          correctIndex: 0,
          challengeType: 'english_illusion',
          challengeTier: 'delirious',
          promptType: 'zh_to_en',
          metaNotes: `【正解考点】${targetWord} (${correctZh})；【干扰幻象】` + (notesList.join('；') || '全英近义形近混淆')
        };
      }

      // 2. 20 <= SAN < 50: 严重污染 · 近义词辨析 (Synonym Confusion)
      if (currentSan < 50 && entry && Array.isArray(entry.synonyms) && entry.synonyms.length > 0) {
        const distractors = [];
        const notes = [];
        for (let item of entry.synonyms) {
          if (distractors.length < 3 && item.zh && item.zh !== correctZh && !distractors.includes(item.zh)) {
            distractors.push(item.zh);
            notes.push(`${item.word} (${item.zh})`);
          }
        }

        if (distractors.length < 3) {
          const fallback = this._collectDistractors(targetWord, correctZh, wordPool);
          for (let f of fallback) {
            if (distractors.length >= 3) break;
            if (!distractors.includes(f) && f !== correctZh) distractors.push(f);
          }
        }
        ...
        return {
          ...
          correctIndex: 0,
          challengeType: 'synonym_confusion',
          challengeTier: 'heavy_polluted',
          promptType: 'word_to_zh',
          metaNotes: `【正解考点】${targetWord} (${correctZh})；【近义辨析】${notes.join('；') || '高混淆近义干扰'}`
        };
      }

      // 3. 50 <= SAN < 80: 微眩侵蚀 · 词形拼写混淆 (Spelling Confusion)
      if (currentSan < 80 && entry && Array.isArray(entry.spelling_confusables) && entry.spelling_confusables.length > 0) {
        const distractors = [];
        const notes = [];
        for (let item of entry.spelling_confusables) {
          if (distractors.length < 3 && item.zh && item.zh !== correctZh && !distractors.includes(item.zh)) {
        ...
        return {
          options: [correctZh, distractors[0], distractors[1], distractors[2]],
          correctIndex: 0,
          challengeType: 'spelling_confusion',
          challengeTier: 'mild_polluted',
          promptType: 'word_to_zh',
          metaNotes: `【正解考点】${targetWord} (${correctZh})；【形近字辨析】${notes.join('；') || '拼写相似易混项'}`
        };
      }

      // 4. SAN >= 80 (或不在试点库时回退): 心神清明 · 常规辨识 (Standard)
    ...
    openEncounterModal(encounter, node) {
      ...
      const isEnglishIllusion = challengeType === 'english_illusion';
      const isSynonymConfusion = challengeType === 'synonym_confusion';
      const isSpellingConfusion = challengeType === 'spelling_confusion';

      const isPolluted = node.type === 'polluted' || encounter.pollutionType === 'synonym_confusion' || isSynonymConfusion || isSpellingConfusion || isEnglishIllusion;
      ...
      if (isBoss) {
        ...
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(239, 68, 68, 0.12); border:1.5px solid rgba(239, 68, 68, 0.45); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            <span style="font-size:24px;">🧠🔥</span>
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#ef4444;">【狂乱绝境 · 纯英全真幻象】 (SAN ${currentSan} · 极度谵妄)</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">心神几近崩溃！中文指引全面失效，形似神似的纯英文伪装逼近，请揪出正解！</div>
            </div>
          </div>
        `;
      } else if (isSynonymConfusion) {
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(147, 51, 234, 0.12); border:1.5px solid rgba(147, 51, 234, 0.4); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            ...
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#a855f7;">【精神污染 · 高混淆近义辨析】 (SAN ${currentSan} · 严重侵蚀)</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">迷雾低语侵蚀，选项全为近义词义项，请精细辨析本词专属核心内涵！</div>
            </div>
          </div>
        `;
      } else if (isSpellingConfusion) {
        bannerHtml = `
          <div class="expedition-mist-banner" style="background:rgba(245, 158, 11, 0.12); border:1.5px solid rgba(245, 158, 11, 0.4); border-radius:var(--radius-md); padding:10px 14px; margin-bottom:12px; display:flex; align-items:center; gap:10px;">
            ...
            <div>
              <div style="font-weight:900; font-size:13.5px; color:#f59e0b;">【微眩侵蚀 · 词形拼写混淆】 (SAN ${currentSan} · 轻度动摇)</div>
              <div style="font-size:11.5px; color:var(--text-secondary); margin-top:2px;">选项已被拼写极度相近的易混词释义替换，凝神辨析细微拼写差异！</div>
            </div>
          </div>
        `;
      } else if (isPolluted) {
      ...
      const wordDisplayHtml = isEnglishIllusion ? `
        <div style="text-align:center; margin-bottom:16px;">
          <div style="display:inline-block; background:rgba(239,68,68,0.12); border:1px solid rgba(239,68,68,0.35); border-radius:12px; padding:2px 12px; font-size:11px; font-weight:800; color:var(--brand-danger); margin-bottom:6px;">
            ⚡ 核心考点释义 · 全英单词四选一
          </div>
          <div class="encounter-target-word" style="font-family:var(--font-serif); font-size:22px; font-weight:900; color:var(--brand-danger); letter-spacing:0.5px;">
            【${escapeHtml(encounter.translation)}】
          </div>
          <div style="font-size:12px; color:var(--text-secondary); margin-top:4px; font-family:var(--font-mono);">
