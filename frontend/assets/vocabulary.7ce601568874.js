...
function updateWordMarkInDOM(word) {
  ...
  // 同步更新吸底工具栏中的 -1 / +1 状态与数值
  const addBtn = document.getElementById('stoneDrawerMarkAdd');
  if (addBtn && currentLookupWord === safeWord) {
    addBtn.textContent = `★ +1 · ${mc}`;
  }
  const minusBtn = document.getElementById('stoneDrawerMarkMinus');
  if (minusBtn && currentLookupWord === safeWord) {
    minusBtn.disabled = (mc <= 0);
    minusBtn.style.opacity = (mc <= 0) ? '0.4' : '1';
    minusBtn.title = (mc <= 0) ? '未标记' : '[-] 减少标记 (Mark -1)';
  }
  const oldDrawerBtn = document.getElementById('stoneDrawerMark');
  if (oldDrawerBtn && currentLookupWord === safeWord) {
    oldDrawerBtn.textContent = `★ 标记 +1 · ${mc}`;
  }
...
function toggleMarkWord(word) {
  if (!word) return;
  const w = word.toLowerCase().trim();
  if (marks[w] && marks[w] > 0) {
    reduceMark(w);
  } else {
    addMark(w);
  }
 ...
 if(/[\u3400-\u9fff]/.test(lf)&&!window.stoneExtraReady){Stone.script('extra-definitions.js').then(()=>{window.stoneExtraReady=true;if(document.getElementById('searchInput').value.trim()===filter.trim())renderWords(filter);}).catch(()=>{});}
  ...
  activeWords.forEach((w, i) => {
    ...
    if (!lf || w.toLowerCase().includes(lf) || (corrected && corrected.toLowerCase().includes(lf)) || (cn && cn.includes(lf)) || String(i + 1) === lf) {
      ...
      const meta = (typeof WORD_UNIT_META !== 'undefined' && WORD_UNIT_META) ? WORD_UNIT_META[w.toLowerCase()] : null;
      const unitBadgeHtml = meta ? `
        <span class="word-unit-badge" title="Unit ${meta.unit} · Lesson ${meta.lesson} · 词群: ${escapeHtml(meta.group_name)} (纸质书第${meta.book_page}页)" onclick="event.stopPropagation(); this.classList.toggle('expanded');">
          <span class="unit-code">U${meta.unit}·L${meta.lesson}</span>
          <span class="unit-group-text">${escapeHtml(meta.group_name)}</span>
        </span>
      ` : '';
      ...
      card.innerHTML = `
        ...
        ${mc > 0 ? `<span class="origami-mark-pill" title="点击减少 1 次标记 (Mark -1)" onclick="event.stopPropagation(); reduceMark('${escapeHtml(w)}');">★ ${mc}</span>` : ''}
      `;
...
function renderMarked() {
  ...
  marked.forEach(([w, count], i) => {
    ...
    card.innerHTML = `
      ...
      <span class="origami-mark-pill" title="点击减少 1 次标记 (Mark -1)" onclick="event.stopPropagation(); reduceMark('${escapeHtml(w)}');">★ ${count}</span>
    `;
    card.tabIndex=0;card.setAttribute('role','group');card.setAttribute('aria-label',w+'，查看释义');
      card.onclick = () => openWordDetails(w);
      card.onkeydown=e=>{if(e.target===card && (e.key==='Enter'||e.key===' ')){e.preventDefault();e.stopPropagation();openWordDetails(w);}};
...
if (typeof window !== 'undefined') {
  ...
  window.reduceMark = reduceMark;
  window.toggleMarkWord = toggleMarkWord;
  window.getMarkCount = getMarkCount;
  window.getMarkedWords = getMarkedWords;
}
