...
function handleSurvivalChoice(opt, idx) {
  ...
  if (isSuccess) {
    ...
    if (typeof appSettings === 'undefined' || appSettings.expeditionAutoReduceMark !== false) {
      reduceMark(word);
    }
    ...
    // 🌍【The World · 万象认知图谱】被动自动触发判定 (连续答对 3 题触发大圆满)
    let worldAutoBadge = '';
    const hasWorldCard = Array.isArray(playerProfile.battleHand) && playerProfile.battleHand.some(c => {
      const o = typeof getCardObject === 'function' ? getCardObject(c) : null;
      return o && (o.name === 'The World' || o.arcana === 'The World' || o.type === 'world_auto');
    });
    if (hasWorldCard && playerProfile.combo > 0 && playerProfile.combo % 3 === 0) {
      const sanGain = 15;
      if (typeof GrayHealthEngine !== 'undefined') {
        GrayHealthEngine.changeSan(playerProfile, sanGain, '世界万象共鸣');
      } else {
        playerProfile.san = Math.min(100, (playerProfile.san || 100) + sanGain);
      }
      let curedGray = 0;
      if (playerProfile.grayHp && playerProfile.grayHp > 0) {
        curedGray = playerProfile.grayHp;
        playerProfile.hp = Math.min(100, (playerProfile.hp || 100) + curedGray);
        playerProfile.grayHp = 0;
      }
      if (typeof restorePlayerFocus === 'function') {
        restorePlayerFocus(1);
      }
      worldAutoBadge = ` · 🌍 世界万象: SAN +${sanGain}${curedGray > 0 ? `·灰血固化 +${curedGray}` : ''}·专注+1`;
    }
    ...
    const recallBadge = (recallResult && recallResult.validRecall && recallResult.recoveredHp > 0)
      ...
      : '';
    showToast(`🎉 战局生还！XP +${gainedXp} · 🪙 +${gainedGold} Gold${riskBadge}${recallBadge}${worldAutoBadge}`);
  } else {
    soundFailure();
...
function renderExpeditionSettlementScroll(summary) {
  if (typeof window !== 'undefined') window.lastExpeditionSummary = summary;
...
function returnToCottageDebrief() {
  soundSuccess();
  const summary = (typeof window !== 'undefined') ? window.lastExpeditionSummary : null;
  if (playerProfile.expedition) {
