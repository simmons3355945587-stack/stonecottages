...
let appSettings = loadFromStorage(STORAGE_KEYS.SETTINGS, {
  ...
  dictLanguageMode: 'zh', // 默认中文详细释义优先（词库列表纯英专注，抽屉详释优先中文）
  showChinese: true,
  expeditionAutoReduceMark: true, // 默认远征闯关答对自动触发减一
  geminiKey: '',
...
if (appSettings.expeditionAutoReduceMark === undefined) {
  appSettings.expeditionAutoReduceMark = true;
}
...
function updateBadges() {
  ...
  const sanTier = (typeof GrayHealthEngine !== 'undefined') 
    ...
    : { label: '清明' };
  if (hudSan) hudSan.innerHTML = `<span class="hud-num">${playerProfile.san}</span><span class="hud-san-tag"> · ${sanTier.label}</span>`;
  ...
  if (hudGold) hudGold.innerHTML = `${playerProfile.gold || 0}<span class="hud-gold-unit"> G</span>`;
...
function openSettingsModal() {
  ...
  const expMarkEl = document.getElementById('cfgExpeditionAutoReduceMark');
  if (expMarkEl) {
    expMarkEl.value = (appSettings.expeditionAutoReduceMark === false) ? 'false' : 'true';
  }
  onDriverModeChange();
...
function saveSettings() {
  ...
  const expMarkEl = document.getElementById('cfgExpeditionAutoReduceMark');
  if (expMarkEl) {
    appSettings.expeditionAutoReduceMark = (expMarkEl.value !== 'false');
  }
...
function updateQuickLangBtn() {
  ...
  if (btn) {
    ...
    btn.innerHTML = isZh ? '🇨🇳 词典:中' : '🇬🇧 词典:英';
    btn.title = isZh ? '当前词典抽屉默认优先中文释义，点击切为纯英词典' : '当前词典抽屉默认优先纯英词典，点击切为中文释义';
  }
}
...
function toggleQuickDictLang() {
  ...
  saveToStorage(STORAGE_KEYS.SETTINGS, appSettings);
  updateQuickLangBtn();
  showToast(next === 'zh' ? '🇨🇳 词典抽屉已设为【中文释义优先】' : '🇬🇧 词典抽屉已设为【纯英权威词典优先】');
}
...
function setupDesktopKeyboardShortcuts() {
  ...
  const handleGlobalKeydown = (e) => {
    ...
    if (isEsc) {
      ...
      else if(visible(document.getElementById('stoneOnboardingModal'))){closeOnboardingTutorial(true);handled=true;}
