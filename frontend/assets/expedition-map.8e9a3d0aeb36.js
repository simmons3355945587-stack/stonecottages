...
(function(root, factory) {
  ...
  const ExpeditionMap = {
    ...
    async getCandidateQuestionsForNode(nodeId, node) {
      ...
      if (typeof Stone !== 'undefined' && typeof Stone.chapter === 'function') {
        try {
      ...
      const beats = (chapterData && Array.isArray(chapterData.beats) && chapterData.beats.length > 0)
        ? chapterData.beats
        : null;
      ...
      if (beats) {
        ...
        for (const beat of selectedBeats) {
          ...
          let correctZh = (typeof chineseDict !== 'undefined' && chineseDict[targetWord])
            ? chineseDict[targetWord]
            : (beat.options && beat.options[0] ? beat.options[0].action_cn : '目标考点释义');
          correctZh = String(correctZh).replace(/\(考纲词汇\)|（考纲词汇）/g, '').trim();

          // 收集候选干扰项 (优先来自同章节其他考词)
          const distractors = [];
          const otherBeats = beats.filter(b => b.targetWord !== targetWord);
          for (let i = 0; i < otherBeats.length && distractors.length < 3; i++) {
            const obWord = otherBeats[i].targetWord;
            let obZh = (typeof chineseDict !== 'undefined' && chineseDict[obWord])
              ? chineseDict[obWord]
              : (otherBeats[i].options && otherBeats[i].options[0] ? otherBeats[i].options[0].action_cn : null);
            if (obZh) {
              obZh = String(obZh).replace(/\(考纲词汇\)|（考纲词汇）/g, '').trim();
              if (obZh !== correctZh && !distractors.includes(obZh)) {
                distractors.push(obZh);
              }
            }
          }
