...
(function() {
  ...
  // 错词与疑难词管理 (Sequential Mistakes & Unclear State)
  let seqMistakesList = []; // Array of { id, word, clean, reason, userTyped, sentence, def, checked }
  let currentLeftTab = 'transcript'; // 'transcript' | 'mistakes'
  const wordDefCache = {};
  let capsuleDismissTimer = null;
  ...
  // Left Panel Switchable Tabs & Mistakes DOMs
  const leftTabTranscript = document.getElementById('leftTabTranscript');
  const leftTabMistakes = document.getElementById('leftTabMistakes');
  const viewLeftTranscript = document.getElementById('viewLeftTranscript');
  const viewLeftMistakes = document.getElementById('viewLeftMistakes');
  const seqMistakesBadge = document.getElementById('seqMistakesBadge');
  const seqSelectAllCheckbox = document.getElementById('seqSelectAllCheckbox');
  const seqSelectedCountText = document.getElementById('seqSelectedCountText');
  const btnClearSeqMistakes = document.getElementById('btnClearSeqMistakes');
  const btnBatchMarkSeq = document.getElementById('btnBatchMarkSeq');
  const seqMistakesListEl = document.getElementById('seqMistakesList');
  const btnQuickMarkUnclear = document.getElementById('btnQuickMarkUnclear');
  const seqMeaningCapsule = document.getElementById('seqMeaningCapsule');
  ...
  function setupSequentialSentence(seg) {
    ...
    // 听写开启时自动切换左侧栏为「错词疑难」面板，沉浸听写不被原文剧透，实时掌握生词积累
    switchLeftPanelTab('mistakes');
  ...
  function handleSequentialTyping(trigger = 'space') {
    ...
    const currentSentence = currentSegments[currentActiveSegIndex]?.t || '';

    if (trigger === 'skip') {
      // 显式点击跳过本词：揭晓词汇并计入通缉令 & 错词表
      if (slot) {
      ...
      addSeqMistake(currentObj.clean, currentObj.w, 'skip', '(已跳过)', currentSentence);
      quizInput.value = '';
      seqCurrentIdx++;
    } else {
      if (!typed) {
      ...
      if (isExact || isSuffix || isMorph) {
        ...
        addSeqMistake(currentObj.clean, currentObj.w, 'wrong', typed, currentSentence);
      }
  ...
  function showToast(msg) {
    ...
    if (!toast) {
      ...
      toast.id = 'appToast';
      toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:rgba(20,25,35,0.92);color:#fff;padding:10px 20px;border-radius:24px;font-size:13.5px;font-weight:700;z-index:99999;box-shadow:0 8px 24px rgba(0,0,0,0.3);display:none;transition:opacity 0.25s,transform 0.25s;pointer-events:none;backdrop-filter:blur(6px);border:1px solid rgba(255,255,255,0.15);';
  ...
  function switchLeftPanelTab(tabName) {
    ...
    if (tabName === 'transcript') {
      if (leftTabTranscript) leftTabTranscript.classList.add('active');
      if (leftTabMistakes) leftTabMistakes.classList.remove('active');
      if (viewLeftTranscript) viewLeftTranscript.style.display = 'flex';
      if (viewLeftMistakes) viewLeftMistakes.style.display = 'none';
    } else {
      if (leftTabTranscript) leftTabTranscript.classList.remove('active');
      if (leftTabMistakes) leftTabMistakes.classList.add('active');
      if (viewLeftTranscript) viewLeftTranscript.style.display = 'none';
      if (viewLeftMistakes) viewLeftMistakes.style.display = 'flex';
      renderSeqMistakesList();
    }
  }

  async function resolveWordDefinition(word) {
    if (!word) return '';
    const clean = word.toLowerCase().trim().replace(/[^a-z]/g, '');
    if (wordDefCache[clean]) return wordDefCache[clean];

    // 1. 优先检索本地 words-data.js 核心字典与元数据
    if (window.chineseDict && window.chineseDict[clean]) {
      const d = window.chineseDict[clean];
      wordDefCache[clean] = d;
      return d;
    }
    if (window.WORD_UNIT_META && window.WORD_UNIT_META[clean] && window.WORD_UNIT_META[clean].definition) {
      const d = window.WORD_UNIT_META[clean].definition;
      wordDefCache[clean] = d;
      return d;
    }

    // 2. 有道词典在线 Suggest API 异步优雅回退
    try {
      const resp = await fetch(`https://dict.youdao.com/suggest?num=1&doctype=json&q=${encodeURIComponent(clean)}`, {
        signal: AbortSignal.timeout(3000)
      });
      if (resp.ok) {
        const data = await resp.json();
        if (data?.data?.entries?.[0]?.explain) {
          const exp = data.data.entries[0].explain;
          wordDefCache[clean] = exp;
          return exp;
        }
      }
    } catch (e) {
  ...
  async function addSeqMistake(cleanWord, origWord, reason = 'wrong', userTyped = '', sentence = '') {
    if (!cleanWord || cleanWord.length < 2) return;

    // 检查是否已有记录：若已有且本次为记不清意思，则升级状态
    const existing = seqMistakesList.find(item => item.clean === cleanWord);
    if (existing) {
      if (reason === 'unclear') existing.reason = 'unclear';
    ...
    const item = {
      ...
      word: origWord || cleanWord,
      clean: cleanWord,
      reason: reason, // 'wrong' | 'skip' | 'unclear'
      userTyped: userTyped,
      sentence: sentence,
      def: wordDefCache[cleanWord] || '释义加载中...',
      checked: true
    };

    seqMistakesList.unshift(item);
    renderSeqMistakesList();

    // 异步拉取词义并刷新展示
    if (!wordDefCache[cleanWord]) {
      const def = await resolveWordDefinition(cleanWord);
      item.def = def;
      renderSeqMistakesList();
    }
  }

  async function markCurrentSeqWordUnclear() {
    if (currentMode !== 'sequential' || seqWords.length === 0) return;
    if (seqCurrentIdx >= seqWords.length) return;

    const cur = seqWords[seqCurrentIdx];
    if (!cur || !cur.clean) return;

    const curSentence = currentSegments[currentActiveSegIndex]?.t || '';

    // 1. 弹出当下释义胶囊（打通音-形-义闭环，不打断听写节奏）
    showMeaningCapsule(cur.clean);

    // 2. 自动收录至左侧错词疑难列表并打上【🤔 记不清意】标签
    await addSeqMistake(cur.clean, cur.w, 'unclear', quizInput ? quizInput.value.trim() : '', curSentence);

    // 3. 自动将左侧栏切换至「错词疑难」面板，给用户明确视觉回馈
    switchLeftPanelTab('mistakes');
  }

  async function showMeaningCapsule(word) {
    if (!seqMeaningCapsule) return;
    const clean = word.toLowerCase().trim().replace(/[^a-z]/g, '');

    let def = wordDefCache[clean] || '正在查询释义...';
    seqMeaningCapsule.innerHTML = `
      <div class="seq-capsule-text">
        <span class="seq-capsule-word">🤔 ${escapeHtml(clean)}:</span>
        <span class="seq-capsule-zh" id="capsuleZhText">${escapeHtml(def)}</span>
      </div>
      <button type="button" class="seq-capsule-close" onclick="dismissMeaningCapsule()" title="关闭">✕</button>
    `;
    seqMeaningCapsule.style.display = 'flex';

    clearTimeout(capsuleDismissTimer);
    capsuleDismissTimer = setTimeout(() => {
      dismissMeaningCapsule();
    }, 4500);

    if (!wordDefCache[clean]) {
      def = await resolveWordDefinition(clean);
      const zhEl = document.getElementById('capsuleZhText');
      if (zhEl) zhEl.textContent = def;
    }
  }

  function dismissMeaningCapsule() {
    clearTimeout(capsuleDismissTimer);
    if (seqMeaningCapsule) seqMeaningCapsule.style.display = 'none';
  }

  function renderSeqMistakesList() {
    if (!seqMistakesListEl) return;

    if (seqMistakesBadge) {
      seqMistakesBadge.textContent = seqMistakesList.length;
    }

    const selectedCount = seqMistakesList.filter(item => item.checked).length;
    if (seqSelectedCountText) {
      seqSelectedCountText.textContent = `已选 ${selectedCount} / ${seqMistakesList.length} 词`;
    }

    if (seqSelectAllCheckbox) {
      seqSelectAllCheckbox.checked = seqMistakesList.length > 0 && selectedCount === seqMistakesList.length;
    }

    if (btnBatchMarkSeq) {
      btnBatchMarkSeq.disabled = (selectedCount === 0);
      btnBatchMarkSeq.innerHTML = `★ 批量加入 Mark 系列 (${selectedCount})`;
    }

    if (seqMistakesList.length === 0) {
      seqMistakesListEl.innerHTML = `
        <div class="seq-mistakes-empty">
          <span>🎧 暂无错词记录</span>
          <small>在顺序模式中听写拼错、跳过或标记【记不清意思】的生词会自动汇集于此，支持一键全选入库！</small>
        </div>
      `;
      return;
    }
    ...
    let html = '';
    seqMistakesList.forEach(item => {
      const tagText = item.reason === 'unclear' ? '🤔 记不清意' : (item.reason === 'skip' ? '⏭️ 已跳过' : '❌ 拼错');
      const tagClass = item.reason === 'unclear' ? 'unclear' : 'wrong';
      const isChecked = item.checked ? 'checked' : '';

      html += `
        <div class="seq-mistake-card ${isChecked}" id="card-${item.id}">
          <div class="seq-mistake-main-row">
            <div class="seq-mistake-left">
              <input type="checkbox" class="seq-mistake-checkbox" ${item.checked ? 'checked' : ''} onchange="toggleSeqMistakeItem('${item.id}', this.checked)">
              <span class="seq-mistake-word">${escapeHtml(item.clean)}</span>
              <span class="seq-mistake-tag ${tagClass}">${tagText}</span>
            </div>
            <div class="seq-mistake-actions">
              <button type="button" class="seq-mistake-icon-btn" onclick="speakWord('${escapeHtml(item.clean)}')" title="发音">🔊</button>
              <button type="button" class="seq-mistake-icon-btn" onclick="markSingleSeqMistake('${escapeHtml(item.clean)}')" title="加入 Mark 生词">★</button>
            </div>
          </div>
          <div class="seq-mistake-zh">${escapeHtml(item.def || '暂无释义')}</div>
          ${item.sentence ? `<div class="seq-mistake-context" title="${escapeHtml(item.sentence)}">“${escapeHtml(item.sentence)}”</div>` : ''}
        </div>
      `;
    });

    seqMistakesListEl.innerHTML = html;
  }

  function toggleSelectAllSeqMistakes(checked) {
    seqMistakesList.forEach(item => { item.checked = checked; });
    renderSeqMistakesList();
  }

  function toggleSeqMistakeItem(id, checked) {
    const item = seqMistakesList.find(x => x.id === id);
    if (item) {
  ...
  function clearSeqMistakes() {
    if (seqMistakesList.length === 0) return;
    seqMistakesList = [];
    renderSeqMistakesList();
    showToast('🗑️ 已清空错词记录');
  }

  function batchAddSeqMistakesToMarks() {
    const selected = seqMistakesList.filter(item => item.checked);
    ...
    let marks = {};
    try {
      marks = JSON.parse(StoneStorage.getItem('vocab_marks_v2') || StoneStorage.getItem('vocab_marks') || '{}');
    } catch (e) {
      marks = {};
    }
    ...
    let customWords = [];
    try {
      customWords = JSON.parse(StoneStorage.getItem('vocab_custom_words') || '[]');
    } catch (e) {
      customWords = [];
    }
    ...
    let words = [];
    try {
      words = JSON.parse(StoneStorage.getItem('vocab_words_v2') || '[]');
    } catch (e) {
      words = [];
    }

    const examId = currentExam ? currentExam.id : 'cet4';
    let addedCount = 0;
    ...
    selected.forEach(item => {
      ...
      if (!w) return;
      marks[w] = (marks[w] || 0) + 1;
      if (!words.includes(w)) words.unshift(w);
      if (!customWords.includes(w)) customWords.push(w);
      addedCount++;

      // 同步通缉令/后台
      reportDeafWordToBackend(w, examId);
    });

    StoneStorage.setItem('vocab_marks_v2', JSON.stringify(marks));
    StoneStorage.setItem('vocab_marks', JSON.stringify(marks));
    StoneStorage.setItem('vocab_custom_words', JSON.stringify(customWords));
    StoneStorage.setItem('vocab_words_v2', JSON.stringify(words));

    showToast(`★ 成功将 ${addedCount} 个词加入小石屋 Mark 生词本！`);

    if (btnBatchMarkSeq) {
      const origText = btnBatchMarkSeq.innerHTML;
      btnBatchMarkSeq.innerHTML = `✓ 已成功入库 (${addedCount})`;
      btnBatchMarkSeq.style.background = 'var(--brand-success)';
      setTimeout(() => {
        btnBatchMarkSeq.innerHTML = origText;
        btnBatchMarkSeq.style.background = '';
      }, 2000);
    }
  }

  function markSingleSeqMistake(word) {
    if (!word) return;
    const clean = word.toLowerCase().trim().replace(/[^a-z]/g, '');
    ...
    showToast(`★ 单词 "${clean}" 已加入 Mark 生词本！`);
  }
  ...
  function setupEventListeners() {
    ...
    quizInput.addEventListener('keydown', (e) => {
      // 快捷键: Alt + M 或在顺序模式输入 ? 标记「🤔 记不清意思」并即时弹显中文释义胶囊
      if ((e.altKey && (e.key === 'm' || e.key === 'M')) || (e.key === '?' && currentMode === 'sequential')) {
        e.preventDefault();
        markCurrentSeqWordUnclear();
        return;
      }
    ...
    if (btnQuickMarkUnclear) {
      btnQuickMarkUnclear.addEventListener('click', markCurrentSeqWordUnclear);
    }
  ...
  // 暴露关键交互接口至 window 对象，完美兼容 HTML inline onclick 触发
  window.switchLeftPanelTab = switchLeftPanelTab;
  window.toggleSelectAllSeqMistakes = toggleSelectAllSeqMistakes;
  window.toggleSeqMistakeItem = toggleSeqMistakeItem;
  window.clearSeqMistakes = clearSeqMistakes;
  window.batchAddSeqMistakesToMarks = batchAddSeqMistakesToMarks;
  window.markCurrentSeqWordUnclear = markCurrentSeqWordUnclear;
  window.dismissMeaningCapsule = dismissMeaningCapsule;
  window.markSingleSeqMistake = markSingleSeqMistake;
