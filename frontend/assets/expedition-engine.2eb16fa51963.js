...
(function(root, factory) {
  ...
  function generateExpeditionGraph(seed) {
    ...
    const nodes = [
      ...
      },
      {
        ...
        rewardXp: 35,
        pollutionType: 'none',
        desc: '猎人踩踏出的坚实小道。避开迷雾干扰，0 SAN 消耗，稳步积累生还基础。',
        status: 'locked',
        nextNodes: [4],
        coords: { x: 30, y: 75 }
      },
      ...
      },
      {
        ...
        name: '破败石塔',
        icon: '🏛️',
        sanCost: towerSanCost,
        rewardGold: 45,
        rewardXp: 60,
        pollutionType: 'time_pressure',
        desc: `断壁残垣散发诡异微光。消耗 ${towerSanCost} SAN，多义深层考点，思考时间紧迫，提供高阶塔罗共鸣。`,
        status: 'locked',
        nextNodes: [7],
        coords: { x: 70, y: 25 }
      },
  ...
  const ExpeditionEngine = {
    ...
    async startExpedition(playerProfile, customSeed = null) {
      ...
      currentSession = {
        ...
        xpEarned: 0,
        healedGrayHp: 0,
        battleHand: Array.isArray(playerProfile.battleHand) ? [...playerProfile.battleHand] : ['The Hermit', 'Temperance', 'The Devil'],
    ...
    async submitAnswer(selectedIndex, usedHint = false) {
      ...
      if (currentSession.hp <= 0) {
        if (currentSession.battleHand.includes('Death') || currentSession.battleHand.includes('The Death')) {
          deathSaved = true;
          currentSession.hp = 10;
          currentSession.grayHp = 0;
          this._applySanChange(-20, '死神牌绝境涅槃');
        } else {
          defeated = true;
    ...
    async campRest() {
      ...
      this._applySanChange(15, '营火温暖休整');
    ...
    async castTacticalTarot(cardName) {
      ...
      if (cardName === 'Justice') {
        ...
        effect = '理智抚慰 +20 SAN';
      } else if (cardName === 'Temperance') {
        effect = '下一次错词自愈翻倍 (+4 HP)';
        ...
        effect = '显露考点线索，生还恢复折半';
      }

      await storage.saveSession(currentSession);
      return { success: true, cardName, effect, currentSan: currentSession.san };
    },
    ...
    async conclude(outcomeType = 'completed') {
      ...
      const summary = {
        ...
        message: isVictory ? '远征大捷！成功突围荒原全线！' : (isEvac ? '见好就收！战利品全数携带归来。' : '远征受挫，已安全护送回石屋。')
      };
