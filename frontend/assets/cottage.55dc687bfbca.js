...
// 7. 手机端智能自动横屏与方向管理 (默认优先保持自然竖屏)
var COTTAGE_ORIENTATION_KEY = 'cottage-orientation-pref';
var cottageOrientationPref = 'portrait';
try {
  const saved = localStorage.getItem(COTTAGE_ORIENTATION_KEY);
  if (saved === 'landscape') {
    cottageOrientationPref = 'landscape';
  } else {
    cottageOrientationPref = 'portrait';
  }
} catch (e) {}

function initCottageOrientation() {
  if (cottageOrientationPref !== 'landscape') {
...
function toggleCottageOrientation() {
  ...
  cottageOrientationPref = isPortraitNow ? 'portrait' : 'landscape';
  try {
  ...
  if (typeof soundClick === 'function') soundClick();
  if (typeof showToast === 'function') {
    showToast(isPortraitNow ? '📱 已切换为竖屏模式' : '🔄 已开启横屏模式');
  }
}
...
function getExpeditionProgressionMode() {
  return (currentExpeditionScope && currentExpeditionScope.progressionMode) ? currentExpeditionScope.progressionMode : 'sequential';
}

function getSequentialWordIndex(sourceKey = 'core') {
  try {
    const raw = (typeof StoneStorage !== 'undefined' ? StoneStorage.getItem(`stone_seq_word_index_${sourceKey}`) : null) ||
                (typeof localStorage !== 'undefined' ? localStorage.getItem(`stone_seq_word_index_${sourceKey}`) : null);
...
function advanceSequentialWordIndex(sourceKey = 'core', amount = 1) {
  try {
    const cur = getSequentialWordIndex(sourceKey);
    const next = cur + amount;
    if (typeof StoneStorage !== 'undefined') {
      StoneStorage.setItem(`stone_seq_word_index_${sourceKey}`, String(next));
    } else if (typeof localStorage !== 'undefined') {
      localStorage.setItem(`stone_seq_word_index_${sourceKey}`, String(next));
    }
    return next;
  } catch (e) {
    return 0;
...
function setExpeditionProgressionMode(mode) {
  if (typeof soundClick === 'function') soundClick();
  currentExpeditionScope.progressionMode = mode;
...
function openExpeditionPrepModal() {
  ...
  if (!modal) {
    ...
    modal.className = 'modal-box expedition-prep-modal';
  ...
  if (typeof Stone !== 'undefined' && typeof Stone.module === 'function') {
    Stone.module('tarot').then(() => {
      const m = document.getElementById('expeditionPrepModal');
      if (m && m.style.display !== 'none') {
        renderExpeditionPrepContent(m);
      }
    }).catch(() => {});
  }
}
...
const LOCAL_TAROT_MAP = {
  'The Magician': { arcana: 'The Magician', name: 'The Magician', skill: '专注时流停滞', icon: '⏳' },
  'The Hermit': { arcana: 'The Hermit', name: 'The Hermit', skill: '语言学微光', icon: '💡' },
  'Death': { arcana: 'Death', name: 'Death', skill: '绝境舍弃追猎', icon: '🦅' },
  'The Devil': { arcana: 'The Devil', name: 'The Devil', skill: '暗夜贪婪契约', icon: '🔥' },
  'The Chariot': { arcana: 'The Chariot', name: 'The Chariot', skill: '破釜沉舟冲刺', icon: '⚡' },
  'The Moon': { arcana: 'The Moon', name: 'The Moon', skill: '幻象理智博弈', icon: '🌙' },
  'The Sun': { arcana: 'The Sun', name: 'The Sun', skill: '晨光免罪庇佑', icon: '☀️' },
  'Justice': { arcana: 'Justice', name: 'Justice', skill: '秩序理智天平', icon: '⚖️' },
  'The Star': { arcana: 'The Star', name: 'The Star', skill: '灵感词性回眸', icon: '✨' },
  'The Empress': { arcana: 'The Empress', name: 'The Empress', skill: '生机灰血温存', icon: '🌿' },
  'The Lovers': { arcana: 'The Lovers', name: 'The Lovers', skill: '连击双生共鸣', icon: '💞' },
  'The Emperor': { arcana: 'The Emperor', name: 'The Emperor', skill: '霸者专注威严', icon: '👑' },
  'The World': { arcana: 'The World', name: 'The World', skill: '全知真理降临', icon: '🌍' },
  'The Fool': { arcana: 'The Fool', name: 'The Fool', skill: '初生盲目坚韧', icon: '🃏' },
  'The High Priestess': { arcana: 'The High Priestess', name: 'The High Priestess', skill: '直觉深渊预知', icon: '🔮' },
  'The Hierophant': { arcana: 'The Hierophant', name: 'The Hierophant', skill: '导师正音法诫', icon: '📿' },
  'Wheel of Fortune': { arcana: 'Wheel of Fortune', name: 'Wheel of Fortune', skill: '命运之轮博弈', icon: '☸️' },
  'The Hanged Man': { arcana: 'The Hanged Man', name: 'The Hanged Man', skill: '逆境忍耐蜕变', icon: '🪢' },
  'Temperance': { arcana: 'Temperance', name: 'Temperance', skill: '心智净化调和', icon: '🕊️' },
  'The Tower': { arcana: 'The Tower', name: 'The Tower', skill: '天崩雷霆碎障', icon: '⚡' },
  'Judgment': { arcana: 'Judgment', name: 'Judgment', skill: '悬赏赏金清算', icon: '🎺' },
  'Strength': { arcana: 'Strength', name: 'Strength', skill: '逆境血怒坚韧', icon: '🦁' }
};

function resolveCardSync(cardRef) {
  if (!cardRef) return null;
  if (typeof cardRef === 'object' && typeof cardRef.then === 'function') return null;
  if (typeof cardRef === 'object' && cardRef.icon && (cardRef.skill || cardRef.name)) return cardRef;
  if (typeof window !== 'undefined' && typeof window.getCardObject === 'function') {
    try {
      const obj = window.getCardObject(cardRef);
      if (obj && typeof obj.then !== 'function' && obj.icon) return obj;
    } catch (_) {}
  }
  const name = typeof cardRef === 'string' ? cardRef : (cardRef.name || cardRef.arcana || cardRef.word || '');
  if (LOCAL_TAROT_MAP[name]) return LOCAL_TAROT_MAP[name];
  if (name) return { arcana: name, name: name, skill: '战术策略', icon: '🃏' };
  return null;
}
...
function renderExpeditionPrepContent(modalEl) {
  ...
  const rawBattleHand = Array.isArray(profile.battleHand) ? profile.battleHand : [];
  const validBattleHand = rawBattleHand.filter(c => c && typeof c.then !== 'function');
  const handCards = validBattleHand.map(resolveCardSync).filter(Boolean);
  ...
  const handSlotsHtml = [0, 1, 2].map(slotIdx => {
    ...
    if (c) {
      const icon = c.icon || '🃏';
      const title = c.arcana || c.name || c.word || '战术手牌';
      const skill = c.skill || '策略效果';
      return `
        <div style="background:var(--paper-surface-sub); border:1.5px solid var(--brand-primary); border-radius:var(--radius-sm); padding:6px 4px; text-align:center;">
          <div style="font-size:9px; color:var(--text-secondary);">槽位 ${slotIdx + 1}</div>
          <div style="font-size:16px; margin:1px 0;">${icon}</div>
          <div style="font-weight:800; font-size:11px; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${escapeHtml(title)}</div>
          <div style="font-size:9.5px; color:var(--brand-primary); font-weight:700;">${escapeHtml(skill)}</div>
        </div>
      `;
    } else {
      return `
        <div style="background:var(--paper-surface-sub); border:1.5px dashed var(--paper-border); border-radius:var(--radius-sm); padding:6px 4px; text-align:center; opacity:0.6;">
          <div style="font-size:9px; color:var(--text-secondary);">槽位 ${slotIdx + 1}</div>
          <div style="font-size:16px; margin:1px 0;">➕</div>
          <div style="font-size:10px; color:var(--text-secondary);">空置</div>
        </div>
      `;
    }
  }).join('');

  const coreWordCount = (typeof getSourceRawWords === 'function' ? getSourceRawWords('core').length : 0) || (typeof CORE_STUDY_WORDS !== 'undefined' ? CORE_STUDY_WORDS.length : 3837);
  const novelWordCount = (typeof getSourceRawWords === 'function' ? getSourceRawWords('novel_ihopethisfindsyouwell').length : 0) || 49;
  const tradeWordCount = (typeof getSourceRawWords === 'function' ? getSourceRawWords('trade_business').length : 0) || 147;
  const allWordCount = (typeof getSourceRawWords === 'function' ? getSourceRawWords('all').length : 0) || (coreWordCount + novelWordCount + tradeWordCount);

  const sourceLabels = {
    core: `📖 核心背诵 (${coreWordCount}词)`,
    novel_ihopethisfindsyouwell: `📚 职场原著 (${novelWordCount}词)`,
    trade_business: `💼 外贸函电 (${tradeWordCount}词)`,
    all: `🌐 全部总库 (全量)`
  };
  ...
  const progMode = getExpeditionProgressionMode();
  const seqIndex = getSequentialWordIndex(currSource);
  const rawScopeWords = (typeof getSourceRawWords === 'function') ? getSourceRawWords(currSource) : [];
  const totalScopeWords = rawScopeWords.length || (currSource === 'core' ? coreWordCount : 3837);
  ...
  modalEl.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1.5px solid var(--paper-border); padding-bottom:10px; margin-bottom:12px;">
      <div style="display:flex; align-items:center; gap:8px;">
        <span style="font-size:22px;">⚔️</span>
        <div>
          <div style="font-weight:900; font-size:16px; color:var(--text-primary);">出征祭坛 · 探险整备 (Expedition Altar)</div>
          <div style="font-size:11px; color:var(--text-secondary);">锁定词库考纲与策略手牌，推门踏上荒原挑战</div>
        </div>
      </div>
      <button class="btn btn-secondary" style="font-size:11px; padding:3px 8px;" onclick="closeExpeditionPrepModal()">✕ 关闭</button>
    </div>

    <!-- 0. 未完成远征断点提示 (断点续玩高亮) -->
    ${inProgress ? `
      <div style="background: rgba(99, 102, 241, 0.08); border: 1.5px solid var(--brand-primary); border-radius: var(--radius-md); padding: 10px 12px; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; gap: 8px; flex-wrap: wrap;">
        <div>
          <div style="font-weight: 800; font-size: 13px; color: var(--brand-primary); display: flex; align-items: center; gap: 6px;">
        ...
        <button class="btn btn-secondary btn-sm" style="font-size: 11px; padding: 3px 8px; color: var(--brand-accent); border-color: var(--brand-accent); font-weight: 700;" onclick="abandonAndEvacuateFromAltar()">
          ⛺ 战术结算返程
        </button>
      </div>
    ` : ''}
    ...
    <!-- 1. 远征考纲与词库范围设置 -->
    ${inProgress ? `
      <div style="background:var(--paper-surface-sub); border:1.5px solid var(--brand-primary); border-radius:var(--radius-md); padding:10px 12px; margin-bottom:12px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
          ...
          <span class="origami-chip" style="background:var(--brand-primary); color:#fff; font-size:10px; padding:2px 8px; border-radius:10px;">
            ${shortSourceLabels[(session.config && session.config.wordSource) || 'core'] || '核心背诵'} ${session.config && session.config.markedOnly ? '· ★ Mark生词' : ''} · ${session.config && session.config.progressionMode === 'sequential' ? '📖顺序闯关' : '🎲乱序探索'}
          </span>
        </div>
        ...
        <div style="font-size:11px; color:var(--text-secondary); line-height:1.4;">
          远征推进中（第 ${(session.clearedNodeIds || []).length + 1} 节点），单词考纲范围已固化锁定。中途可随时【战术结算返程】。
        </div>
      </div>
    ` : `
      <div style="background:var(--paper-surface-sub); border:1.5px solid var(--paper-border); border-radius:var(--radius-md); padding:10px 12px; margin-bottom:12px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
          <div style="display:flex; align-items:center; gap:6px;">
            <span style="font-size:15px;">🎯</span>
            <span style="font-size:12.5px; font-weight:800; color:var(--text-primary);">出征考纲与词库范围</span>
          </div>
          <span class="origami-chip" style="background:var(--brand-accent); color:#fff; font-size:9.5px; padding:1px 6px; border-radius:8px;">
            7 节点分支
          </span>
        </div>
        ...
        <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:6px; margin-bottom:8px;">
          <button type="button" class="btn scope-chip-btn ${currSource === 'core' ? 'btn-primary' : 'btn-secondary'}" style="height:34px; font-size:11px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 4px;" onclick="setExpeditionPrepScope('core')">
            📖 核心背诵 (${coreWordCount}词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'novel_ihopethisfindsyouwell' ? 'btn-primary' : 'btn-secondary'}" style="height:34px; font-size:11px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 4px;" onclick="setExpeditionPrepScope('novel_ihopethisfindsyouwell')">
            📚 职场原著 (${novelWordCount}词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'trade_business' ? 'btn-primary' : 'btn-secondary'}" style="height:34px; font-size:11px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 4px;" onclick="setExpeditionPrepScope('trade_business')">
            💼 外贸函电 (${tradeWordCount}词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'all' ? 'btn-primary' : 'btn-secondary'}" style="height:34px; font-size:11px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 4px;" onclick="setExpeditionPrepScope('all')">
            🌐 全部总库 (全量)
          </button>
        </div>

        <!-- 勾选只挑战 mark 词 -->
        <div style="background:var(--paper-surface); border:1.5px solid ${markedOnly ? 'var(--brand-accent)' : 'var(--paper-border)'}; border-radius:var(--radius-sm); padding:6px 10px; display:flex; align-items:center; justify-content:space-between; gap:8px;">
          <label style="display:flex; align-items:center; gap:6px; cursor:pointer; font-size:11.5px; font-weight:800; color:var(--text-primary); margin:0; user-select:none;">
            <input type="checkbox" id="chkExpeditionMarkedOnly" ${markedOnly ? 'checked' : ''} onchange="toggleExpeditionPrepMarkedOnly(this.checked)" style="width:15px; height:15px; accent-color:var(--brand-accent); cursor:pointer;">
            <span>⭐ 仅挑战生词本 (只出 Mark 重点词)</span>
          </label>
          <span id="prepMarkedCountBadge" style="font-size:10.5px; font-weight:800; color:${scopeMarkedCount > 0 ? 'var(--brand-accent)' : 'var(--text-secondary)'};">
            ${scopeMarkedCount > 0 ? `该范围已标 ${scopeMarkedCount} 词` : '该库暂无标记'}
          </span>
        </div>

        <!-- 顺序大纲闯关 vs 乱序探索模式选择 -->
        <div style="background:var(--paper-surface); border:1.5px solid var(--paper-border); border-radius:var(--radius-sm); padding:8px 10px; margin-top:6px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
            <span style="font-size:11.5px; font-weight:800; color:var(--text-primary);">出题节奏</span>
            <span style="font-size:10.5px; font-weight:700; color:var(--brand-primary);">
              ${progMode === 'sequential' ? `第 ${Math.min(seqIndex + 1, totalScopeWords)} 词 / 共 ${totalScopeWords} 词` : '🎲 乱序探索'}
            </span>
          </div>
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:6px;">
            <button type="button" class="btn ${progMode === 'sequential' ? 'btn-primary' : 'btn-secondary'}" style="height:30px; font-size:10.5px; font-weight:800; padding:0 4px;" onclick="setExpeditionProgressionMode('sequential')">
              📖 顺序大纲逐词闯关
            </button>
            <button type="button" class="btn ${progMode === 'random' ? 'btn-primary' : 'btn-secondary'}" style="height:30px; font-size:10.5px; font-weight:800; padding:0 4px;" onclick="setExpeditionProgressionMode('random')">
              🎲 乱序随机探索
            </button>
          </div>
        </div>
    ...
    <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:6px; margin-bottom:10px;">
      <div style="background:var(--paper-surface); border:1px solid var(--paper-border); border-radius:var(--radius-sm); padding:6px 4px; text-align:center;">
        <div style="font-size:9.5px; color:var(--text-secondary);">生命状态</div>
        <div style="font-size:12.5px; font-weight:900; color:var(--brand-danger);">❤️ ${profile.hp} HP</div>
      </div>
      <div style="background:var(--paper-surface); border:1px solid var(--paper-border); border-radius:var(--radius-sm); padding:6px 4px; text-align:center;">
        <div style="font-size:9.5px; color:var(--text-secondary);">心智清明</div>
        <div style="font-size:12.5px; font-weight:900; color:var(--brand-primary);">🧠 ${profile.san} SAN</div>
      </div>
      <div style="background:var(--paper-surface); border:1px solid var(--paper-border); border-radius:var(--radius-sm); padding:6px 4px; text-align:center;">
        <div style="font-size:9.5px; color:var(--text-secondary);">随身金币</div>
        <div style="font-size:12.5px; font-weight:900; color:var(--brand-accent);">🪙 ${profile.gold || 0} Gold</div>
      </div>
    </div>
    ...
    <div style="margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
        <span style="font-size:11.5px; font-weight:800; color:var(--text-primary);">🎴 携带策略手牌 (${handCards.length}/3)</span>
        <button class="btn btn-secondary" style="font-size:10px; padding:1px 6px;" onclick="closeExpeditionPrepModal(); openDeckSelectorModal();">🔄 调整构筑背包</button>
      </div>
    ...
    <!-- 4. 路线前瞻提示 -->
    <div style="background:rgba(99, 102, 241, 0.05); border:1px dashed rgba(99, 102, 241, 0.25); border-radius:var(--radius-sm); padding:8px 10px; font-size:11px; color:var(--text-secondary); line-height:1.4; margin-bottom:12px;">
      💡 <strong>路线抉择提示</strong>：第 1 节点安全启程；后续节点可自由抉择 🌲安全小径 (0 SAN) / 🌫️迷雾支路 (-10 SAN) / 🏛️禁忌遗迹 (-15 SAN)；中途随时可【见好就收安全撤退】100% 携带收益归来。
    </div>

    <!-- 5. 底部吸顶常驻行动按钮 (免反复划拉) -->
    <div class="prep-modal-sticky-footer">
      <button class="btn btn-primary" style="flex:2; height:44px; font-size:13.5px; font-weight:800; box-shadow: 0 4px 14px rgba(99, 102, 241, 0.3);" onclick="startExpeditionFromAltar()">
        ${inProgress ? `🚪 继续未竟的远征 (第 ${(session.clearedNodeIds || []).length + 1} 节点) ➔` : '🚪 推门踏上出征 ➔'}
      </button>
      <button class="btn btn-secondary" style="flex:1; height:44px; font-size:11.5px;" onclick="closeExpeditionPrepModal(); startSafeStudyWithErrors([]);">
        📚 书架安全研习 (0扣血)
      </button>
    </div>
  `;
  ...
  if (typeof ExpeditionEngine !== 'undefined') {
    if (!ExpeditionEngine.getSession()) {
      ...
      await ExpeditionEngine.startExpedition(profile, null, {
        ...
        progressionMode: scope.progressionMode || 'sequential'
      });
    }
  }
...
if (typeof window !== 'undefined') {
  ...
  window.getExpeditionProgressionMode = getExpeditionProgressionMode;
  window.getSequentialWordIndex = getSequentialWordIndex;
  window.advanceSequentialWordIndex = advanceSequentialWordIndex;
  window.setExpeditionProgressionMode = setExpeditionProgressionMode;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    getExpeditionProgressionMode,
    getSequentialWordIndex,
    advanceSequentialWordIndex,
    setExpeditionProgressionMode,
