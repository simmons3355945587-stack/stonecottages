    ...
    ` : `
      <div style="background:var(--paper-surface-sub); border:1.5px solid var(--paper-border); border-radius:var(--radius-md); padding:14px; margin-bottom:14px;">
        ...
        <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:8px; margin-bottom:10px;">
          <button type="button" class="btn scope-chip-btn ${currSource === 'core' ? 'btn-primary' : 'btn-secondary'}" style="height:36px; font-size:11.5px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 6px;" onclick="setExpeditionPrepScope('core')">
            📖 核心背诵 (480词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'novel_ihopethisfindsyouwell' ? 'btn-primary' : 'btn-secondary'}" style="height:36px; font-size:11.5px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 6px;" onclick="setExpeditionPrepScope('novel_ihopethisfindsyouwell')">
            📚 职场原著 (49词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'trade_business' ? 'btn-primary' : 'btn-secondary'}" style="height:36px; font-size:11.5px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 6px;" onclick="setExpeditionPrepScope('trade_business')">
            💼 外贸函电 (147词)
          </button>
          <button type="button" class="btn scope-chip-btn ${currSource === 'all' ? 'btn-primary' : 'btn-secondary'}" style="height:36px; font-size:11.5px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:4px; padding:0 6px;" onclick="setExpeditionPrepScope('all')">
            🌐 全部总库 (全量)
          </button>
        </div>
        ...
        <div style="background:var(--paper-surface); border:1.5px solid ${markedOnly ? 'var(--brand-accent)' : 'var(--paper-border)'}; border-radius:var(--radius-sm); padding:8px 12px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
          ...
          <span id="prepMarkedCountBadge" style="font-size:11px; font-weight:800; color:${scopeMarkedCount > 0 ? 'var(--brand-accent)' : 'var(--text-secondary)'};">
            ${scopeMarkedCount > 0 ? `该范围已标 ${scopeMarkedCount} 词` : '该库暂无标记'}
          </span>
        </div>

        <!-- 顺序大纲闯关 vs 乱序探索模式选择 -->
        <div style="background:var(--paper-surface); border:1.5px solid var(--paper-border); border-radius:var(--radius-sm); padding:10px 12px; margin-top:8px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
            <span style="font-size:12px; font-weight:800; color:var(--text-primary);">📖 单词出题节奏模式</span>
            <span style="font-size:11px; font-weight:700; color:var(--brand-primary);">
              ${progMode === 'sequential' ? `大纲进度：第 ${Math.min(seqIndex + 1, totalScopeWords)} 词 / 共 ${totalScopeWords} 词` : '🎲 乱序探索'}
            </span>
          </div>
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
            <button type="button" class="btn ${progMode === 'sequential' ? 'btn-primary' : 'btn-secondary'}" style="height:32px; font-size:11px; font-weight:800; padding:0 6px;" onclick="setExpeditionProgressionMode('sequential')">
              📖 顺序大纲逐词闯关
            </button>
            <button type="button" class="btn ${progMode === 'random' ? 'btn-primary' : 'btn-secondary'}" style="height:32px; font-size:11px; font-weight:800; padding:0 6px;" onclick="setExpeditionProgressionMode('random')">
              🎲 乱序随机探索
            </button>
          </div>
          <div style="font-size:10.5px; color:var(--text-secondary); margin-top:6px; line-height:1.4;">
            ${progMode === 'sequential' ? '✨ 严格按大纲词表序号逐词向后探索，单次远征跨层 100% 防重复，持续累计掌握进度。' : '🎲 在目标词库中随机洗牌探索，单次远征内同样保证 0 重复词。'}
          </div>
        </div>
        ...
        <div style="font-size:11px; color:var(--text-secondary); margin-top:6px; line-height:1.5;">
          ${markedOnly ? (
            scopeMarkedCount > 0
              ? `🎯 <strong style="color:var(--brand-accent);">重点突击模式</strong>：推门出征将严格从这 <strong>${scopeMarkedCount} 个 Mark 生词</strong> 中抽选考题，集中消除盲区！<span style="color:var(--text-secondary); font-size:10.5px;">（${(typeof appSettings !== 'undefined' && appSettings && appSettings.expeditionAutoReduceMark === false) ? '🔒 答对不减 Mark' : '★ 答对自动减 1'} · 系统设置中可自由切换）</span>`
              : `⚠️ <strong style="color:var(--brand-danger);">提示</strong>：所选词库暂无 Mark 标记（全局生词本共 ${totalMarked} 词）。出征时将尝试使用全局标记词或全量词库。`
          ) : `🌲 <strong style="color:var(--brand-primary);">全量探索模式</strong>：推门出征将在所选词库全量词汇中按选定模式生成关卡考题。`}
        </div>
      </div>
    `}
