  ...
  modalEl.innerHTML = `
    ...
    <div style="background:var(--paper-surface-sub); border:1px solid var(--paper-border); border-radius:var(--radius-md); padding:12px 14px; margin-bottom:14px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
        <span style="font-size:11px; font-weight:800; color:var(--brand-accent);">📖 远征拓扑目标</span>
        <span class="origami-chip" style="background:var(--brand-accent); color:#fff; font-size:10px; padding:2px 8px; border-radius:10px;">7 节点分支拓扑 · 3~5分钟</span>
      </div>
      <div style="font-size:14px; font-weight:800; color:var(--text-primary); margin-bottom:4px;">职场机密与危机周旋 · 原著高频考点</div>
      <div style="font-size:11.5px; color:var(--text-secondary); line-height:1.5;">从安全小径启程，途经迷雾支路与禁忌遗迹分支，挑战易混高频词，击破守关领主斩获全额大捷赏金！</div>
    </div>
...
async function startExpeditionFromAltar() {
  ...
  if (typeof ExpeditionEngine !== 'undefined') {
    if (!ExpeditionEngine.getSession()) {
      await ExpeditionEngine.startExpedition(profile);
    }
  }
