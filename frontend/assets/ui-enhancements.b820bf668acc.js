...
(() => {
  ...
  if(!listening){
    ...
    for(const [id,name,label] of [['navCottage','cottage','石屋大厅'],['navWords','words','词库'],['navSurvival','games','小说闯关'],['navMatch','games','消消乐'],['navProfile','games','塔罗卡牌'],['navCet4','listening','四级精听']]){const b=document.getElementById(id);b.querySelector('.nav-icon').innerHTML=icon(name);b.querySelector('span:last-child').textContent=label;}
    ...
    const syncNavigation=()=>{const active=document.querySelector('.view-section.active')?.id;const name={viewCottage:'cottage',viewWords:'words',viewSurvival:'survival',viewMatch:'match',viewProfile:'profile'}[active]||'cottage';document.body.dataset.activeView=name;if(!['cottage','words'].includes(name)){preferences.lastGame=name;save();}mobileNav.querySelectorAll('[data-main-nav]').forEach(b=>{if(b.dataset.mainNav===(name==='words'?'words':'games'))b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});gameNav.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.game===name)));};
    ...
    const route=new URLSearchParams(location.search).get('view');if(['cottage','survival','match','profile','words'].includes(route))switchNavView(route);else if(route==='mine')openAccount();else if(route==='marked')switchNavView('words').then(()=>switchSubTab('marked'));else switchNavView('cottage');
