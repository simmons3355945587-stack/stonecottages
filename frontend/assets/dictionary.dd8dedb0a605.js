...
async function openWordDetails(word) {
 ...
 if(!CORE_STUDY_WORDS.includes(safeWord)){try{await Stone.script('extra-definitions.js');}catch{}}
  ...
  const meta = (typeof WORD_UNIT_META !== 'undefined' && WORD_UNIT_META) ? (WORD_UNIT_META[lookup] || WORD_UNIT_META[safeWord]) : null;
  const unitMetaHtml = meta ? `
    <div style="font-size: 12px; color: var(--brand-primary); margin-top: 6px; display: inline-flex; align-items: center; gap: 6px; background: rgba(99,102,241,0.08); border: 1px solid rgba(99,102,241,0.22); border-radius: var(--radius-sm); padding: 3px 8px;">
      <strong>Unit ${meta.unit} · Lesson ${meta.lesson}</strong>
      <span>词群：${escapeHtml(meta.group_name)}</span>
      <span style="color: var(--text-secondary); font-size: 11px;">(${meta.type} · 纸质书第 ${meta.book_page} 页)</span>
    </div>
  ` : '';
