const fs=require('fs'),path=require('path'),crypto=require('crypto');const terser=require('terser');const dir=path.join(__dirname,'frontend');
async function build(){
 ...
 for(const f of all){let data=fs.readFileSync(f);if(f.endsWith('.js')){const r=await terser.minify(data.toString(),{compress:false,mangle:false,format:{comments:false}});data=Buffer.from(r.code);}
